...
var tests = [
  taskSetupEwsServer,
  taskUpdateTrashFolder,
  taskShutdown,
];

function* taskUpdateTrashFolder() {
  let trashFolder = gEwsIncomingServer.rootFolder.getFolderWithFlags(Ci.nsMsgFolderFlags.Trash);
  dl("trashFolder is " + trashFolder.prettyName);
  let urlListener = new PromiseUtils.UrlListener();
  safeGetInterface(trashFolder, Ci.msqIEwsMailFolder).updateFolderWithListener(null, urlListener);

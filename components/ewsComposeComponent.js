...
XPCOMUtils.defineLazyModuleGetter(this, "JaBaseCompose",
...
EwsCompose.prototype = {
  ...
  sendMsgToServer(msgType, identity, accountKey)
  {
    ...
    */
    ...
    {
      let folder = MailUtils.getFolderForURI(destinationURI);
      if (folder)
    ...
    let ewsServer = safeGetInterface(outgoing, Ci.msqIEwsIncomingServer);
    if (ewsServer)
    {
  ...
  } catch(e) {log.error(se(e));} // logs error without throwing, so falls through to normal send
  ...
  { try {
    ...
    ewsSend = safeGetInterface(ewsSend, Ci.nsISupports).wrappedJSObject;

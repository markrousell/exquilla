...
XPCOMUtils.defineLazyModuleGetter(this, "JaBaseMsgFolder", 
...
function stringLineStreamFactory(aString) {
  ...
  Object.defineProperty(lineStream, 'QueryInterface', {
    value: XPCOMUtils.generateQI([Ci.nsIInputStream,
...
function addressesToHeader(properties, plName, msgHdr, msgHdrName) {
  ...
  do {
    let mailboxes = properties.getPropertyLists(plName)
                              .enumerate();
    while (mailboxes.hasMoreElements()) {
      let mailboxPL = mailboxes.getNext().QueryInterface(Ci.msqIPropertyList).wrappedJSObject;
...
// termination event StopCopy from an msqIEwsEventListener
...
// Add an additional interface only needed by this custom class.
  extraInterfaces: [ Ci.msqIEwsMailFolder ],
}
...
EwsMsgFolder.prototype = {
  ...
  // msqIEwsMailFolder implementation

  get nativeMailbox() {
    let exqServer = safeGetInterface(this.server, Ci.msqIEwsIncomingServer);
  ...
  updateFolderWithListener(aMsgWindow, urlListener) {
    Task.spawn(function* ()
    {
      ...
      let ewsServer = safeGetInterface(this.server, Ci.msqIEwsIncomingServer);
      if (ewsServer.unavailable != Ci.msqIEwsIncomingServer.AVAILABLE)
      {
        ...
        let result = yield urlListener.promise;
      ...
      if (!syncState) {
        ...
        let reindexResult = yield reindexListener.promise;
      ...
      if (!db || !db.summaryValid)
      {
        ...
        let result = yield resyncListener.promise;
      ...
      let discoverFoldersResult = yield discoverFoldersListener.promise;
      ...
      let getNewListener = new PromiseUtils.MachineListener(
        (aItem, aEvent, aData, aResult) => {
          ...
          if (aEvent == "ItemChanged") {
            ...
            let item = aData.QueryInterface(Ci.msqIEwsNativeItem);
            ...
            if (item.flags & (Ci.msqIEwsNativeItem.DeletedBit | Ci.msqIEwsNativeItem.DeletedOnServerBit))
            ...
            datastoreListener.promise.then(
              (result) => {
                if (result.status != Cr.NS_OK)
  ...
  updateFromNative(aNativeFolder)
  {
    ...
    while (subfoldersEnum.hasMoreElements()) {
      ...
      let childEwsMailFolder = safeGetInterface(childSkinkFolder, (Ci.msqIEwsMailFolder));
              ...
              !childClass)) // this folder not updated from locals
        {
          ...
          if (childMailFolder)
          {
            if (!childMailFolder.getFlag(Ci.nsMsgFolderFlags.Virtual))
            {
              ...
              let childEwsMailFolder = safeGetInterface(childMailFolder, Ci.msqIEwsMailFolder);
  ...
  resyncFromNative(aFixProblems, urlListener) {
    Task.spawn(function* ()
    {
      ...
      let getIdsResult = yield getIdsListener.promise;
      if (getIdsResult.status != Cr.NS_OK)
        throw CE("GetAllServerIds failed");

      let serverItems = getIdsResult.item.QueryInterface(Ci.msqIPropertyList).wrappedJSObject;
      ...
      let getFolderIdsResult = yield getFolderIdsListener.promise;
      ...
      let itemIds = getFolderIdsResult.data.QueryInterface(Ci.msqIStringArray).wrappedJSObject;
      let changeKeys = getFolderIdsResult.item.QueryInterface(Ci.msqIStringArray).wrappedJSObject;
      ...
      while (skinkEnum.hasMoreElements()) {
        ...
        yield new Promise( (resolve) => Services.tm.mainThread.dispatch(resolve, Ci.nsIThread.DISPATCH_NORMAL));
      ...
      for (let i = 0; i < missingIds.length; i++) {
        ...
        let onDoneResult = yield onDoneListener.promise;
        if (onDoneResult.status != Cr.NS_OK) {
          log.config("error getting item from database");
          continue;
        }

        let item = onDoneResult.item.QueryInterface(Ci.msqIEwsNativeItem);
        ...
        if (item.flags & (Ci.msqIEwsNativeItem.AllLocally |
                          Ci.msqIEwsNativeItem.Dirty |
                          Ci.msqIEwsNativeItem.HasTempId)) {
          log.debug("Message with flags needing update, flags are (hex): " + item.flags.toString(16));
          continue;
        }

        item.raiseFlags(Ci.msqIEwsNativeItem.NewOnServerBit);
      ...
      for (let resyncId of resyncIds) {
        ...
        let result = yield onDoneListener.promise;
        if ( (result.status != Cr.NS_OK) || !result.item) {
          log.warn("Could not get item, skipping resync");
          continue;
        }
        let item = result.item.QueryInterface(Ci.msqIEwsNativeItem);
        item.raiseFlags(Ci.msqIEwsNativeItem.NeedsResync);
        let datastoreListener = new PromiseUtils.DatastoreListener;
        datastore.putItem(item, datastoreListener);
        yield datastoreListener.promise;
      }
      ...
      for (let itemIndex of dbMissingIds) {
        ...
        missingItem.raiseFlags(Ci.msqIEwsNativeItem.NeedsResync);
        let putItemListener = new PromiseUtils.DatastoreListener();
  ...
  reindexExperiment(aWindow, urlListener) {
    ...
    Task.spawn(function* ()
    {
      ...
      if (this.getStringProperty("Exquilla-Reindexing") != "true") {
        ...
        let setSyncResult = yield setSyncListener.promise;
  ...
  reindex(aWindow, urlListener) {
    ...
    Task.spawn(function* ()
    {
      ...
      do { // break when done or error
        ...
        let ewsParent = safeGetInterface(this.parent, Ci.msqIEwsMailFolder);
        if (!ewsParent)
          break;
      ...
      let getNewListener = new PromiseUtils.MachineListener(
        (aItem, aEvent, aData, aResult) => {
          ...
          {
            let item = aData.QueryInterface(Ci.msqIEwsNativeItem);
            if (!(item.processingFlags & Ci.msqIEwsNativeItem.DidNotChange)) {
              this._reconcileItem(item, true);
              ...
              let datastoreListener = new PromiseUtils.DatastoreListener();
              mailbox.datastore.putItem(item, datastoreListener);
              promises.push(datastoreListener.promise.then(
                (result) => {
                  if (result.status != Cr.NS_OK)
                    log.warn("Error saving item in database");
                }, (ex) => {
      ...
      mailbox.getNewItems(nativeFolder, getNewListener);
      let result = yield getNewListener.promise;
...
/**/
    ...
    for (let i = 0; i < messages.length; ++i)
    {
      ...
      if (itemId)
      {
        ...
        if (nativeItem)
        {
          ...
          //nativeItem->ClearFlags(msqIEwsNativeItem::AllOnServer);
        }
      }
      else
  ...
  copyLocalMessages(aSrcMessages, aDestMessages, aNewItems) {
    ...
    for (let i = 0; i < messageCount; ++i)
    {
      ...
      let nativeItem = aNewItems.queryElementAt(i, Ci.msqIEwsNativeItem);
      if (!nativeItem)
      {
      ...
      nativeItem.clearFlags(Ci.msqIEwsNativeItem.AllOnServer);
    }
    this.enableNotifications(Ci.nsIMsgFolder.allMessageCountNotifications, true, true);
  },
  ...
  copyFolderLocal(srcFolder, isMoveFolder, msgWindow, listener)
  {
    ...
    let ewsNewFolder = safeGetInterface(newMsgFolder, Ci.msqIEwsMailFolder);
    {
  ...
  downloadAllForOffline(urlListener, msgWindow) {
    ...
    Task.spawn(function* ()
    {
      ...
      while(messagesEnum.hasMoreElements()) {
        ...
        let onDoneResults = yield onDoneListener.promise;
        if (!(CS(onDoneResults.status))) {
          log.warn("Failed to get item from datastore");
          continue;
        }
        let nativeItem = onDoneResults.item.QueryInterface(Ci.msqIEwsNativeItem);
        ...
        do {
          ...
          if (!nativeItem.properties ||
              ...
              !(nativeItem.flags & Ci.msqIEwsNativeItem.HasOfflineBody)) {
            getMessage = true;
            break;
          }
  ...
  addSubfolder(name) {
    ...
    for (let itrial = 1; itrial <= 5; itrial++) {
      try {
        ...
        log.error("Could not create subfolder " + name + " Error: " + e);
  ...
  fetchMsgPreviewText(aKeysToFetch, aNumKeys, aLocalOnly, aUrlListener)
  {
    ...
    for (let key of aKeysToFetch)
    {
      ...
      let body;
      if (nativeItem.processingFlags & Ci.msqIEwsNativeItem.HasBody)
      {
        body = nativeItem.body;
      }
      else if (nativeItem.flags & Ci.msqIEwsNativeItem.HasOfflineBody)
      {
      ...
      else
      {
        ...
        if (nativeItem.flags & Ci.msqIEwsNativeItem.BodyIsHtml)
  ...
  getMsgInputStream(aMsgHdr, aReusableObject)
  {
    try {
      ...
      {
        let internetMessageHeader = headers.queryElementAt(index, Ci.msqIPropertyList).wrappedJSObject;
  ...
  emptyTrash(msgWindow, urlListener)
  {
    Task.spawn(function* ()
    {
  ...
  createStorageIfMissing(urlListener) {
    Task.spawn(function* ()
    {
  ...
  copyMessages(srcFolder, messages, isMove, msgWindow, copyServiceListener, isFolder, allowUndo) {
    ...
    Task.spawn(function* ()
    {
      ...
      if (srcServerURI == myServerURI) {
        this.enableNotifications(Ci.nsIMsgFolder.allMessageCountNotifications, false, false);
        let result = yield this._promiseCopyEwsMessages(srcFolder, messages, isMove);
        if (!CS(result))
          throw CE("Error return returned from promiseCopyEwsMessages: " + result);
      }
      else {
        let result = yield this._promiseCopyAnyMessages(srcFolder, messages, isMove);
        if (!CS(result))
  ...
  copyFileMessage(file, msgToReplace, isDraft, newMsgFlags,
    ...
    Task.spawn(function* ()
    {
      ...
      let result = yield this._promiseCopyFileMessage(file, msgToReplace,
  ...
  copyFolder(srcFolder, isMoveFolder, msgWindow, copyServiceListener) {
    ...
    Task.spawn(function* ()
    {
      ...
      let listener = new PromiseUtils.MachineListener();
      let folderIds = new StringArray();
      folderIds.append(safeGetInterface(srcFolder, Ci.msqIEwsMailFolder).folderId);
  ...
  deleteMessages(messages, msgWindow, deleteStorage, isMove, copyServiceListener, allowUndo) {
    ...
    Task.spawn(function* ()
    {
      ...
      let result = yield deleteListener.promise;
      if (CS(result.status)) {
  ...
  _promiseCopyFileMessage: Task.async(function*
    (aFile, aMsgToReplace, aIsDraft, aNewMsgFlags, aKeywords, aMsgWindow, aListener) {

    // read the file into a string
    let decoder = new TextDecoder();
    let array = yield OS.File.read(aFile.path);
    ...
    let result = yield machineListener.promise;
    if (!CS(result.status))
  ...
  _promiseCopyAnyMessages: Task.async(function*
    (srcFolder, messages, isMove) {
      ...
      for (let key = keys.pop(); key; key = keys.pop()) {
        ...
        let result = yield inputStream.promise;
        ...
        let createResult = yield createListener.promise;
      ...
      if (isMove) {
        ...
        let deleteResult = yield promiseDeleteDone;
  ...
  _promiseCopyEwsMessages: Task.async(function*
    (srcFolder, messages, isMove) {
      ...
      let srcEwsFolder = safeGetInterface(srcFolder, Ci.msqIEwsMailFolder);
  ...
  } catch (e) { log.error("_finishCopy error: " + e);}},
  ...
  _createSubFolders(path) {
    ...
    while (directoryEnumerator.hasMoreElements())
    {
      ...
      if (child)
      {
        ...
        // We store the folder name in a JSON file owned by the server.
        let exqServer = safeGetInterface(child.server, Ci.msqIEwsIncomingServer);
  ...
  _updateNativeFromSkink() {
    ...
    if (parent)
    {
      let ewsParent = safeGetInterface(parent, Ci.msqIEwsMailFolder);
  ...
  _updateItemFromNative(hdr, aDatabase, aItem, aNotify) {
    ...
    let extendedProperties = properties.getPropertyLists("ExtendedProperty")
                                       .enumerate();
    while (extendedProperties.hasMoreElements()) {
      let pl = extendedProperties.getNext().QueryInterface(Ci.msqIPropertyList).wrappedJSObject;
  ...
  { try {
    ...
    aNativeItem.clearFlags(Ci.msqIEwsNativeItem.NewOnServerBit |
                           Ci.msqIEwsNativeItem.UpdatedOnServerBit |
                           Ci.msqIEwsNativeItem.NeedsResync);

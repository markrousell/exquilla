                       ...
                       msgWindow)    // nsIMsgWindow
{
  ...
  let ewsUrl = uri.QueryInterface(Ci.nsIInterfaceRequestor)
                  .getInterface(Ci.msqIEwsUrl);
...
EwsService.prototype = {
               ...
               aLocalOnly)       // boolean
  {
    ...
    if (aLocalOnly)
    {
      if (!ewsUrl.mailbox ||
          !ewsUrl.itemId ||
          !(ewsUrl.mailbox.getItem(ewsUrl.itemId).flags & Ci.msqIEwsNativeItem.HasOfflineBody))
      {
    ...
    if (aDisplayConsumer instanceof Ci.nsIDocShell) {
      let loadInfoObj = {};
      aDisplayConsumer.createLoadInfo(loadInfoObj);
      let loadInfo = loadInfoObj.value;
      loadInfo.loadType = Ci.nsIDocShellLoadInfo.loadNormal;
      uri.loadURI(aDisplayConsumer, loadInfo, Ci.nsIWebNavigation.LOAD_FLAGS_NONE);
      return uri;
    }
...
//  nsIMsgProtocolInfo implementation
  ...
  get serverIID() { return Components.interfaces.msqIEwsIncomingServer; },
...
*/
  ...
  openAttachment(aContentType, aFileName, aUrl, aMessageURI, aDisplayConsumer, aMsgWindow, aUrlListener)
  {
    ...
    if (aDisplayConsumer instanceof Ci.nsIDocShell) {
      ...
      aDisplayConsumer.createLoadInfo(loadInfoObj);
      let loadInfo = loadInfoObj.value;
      loadInfo.loadType = Ci.nsIDocShellLoadInfo.loadLink;
      uri.loadURI(aDisplayConsumer, loadInfo, Ci.nsIWebNavigation.LOAD_FLAGS_NONE);
      return;
    }

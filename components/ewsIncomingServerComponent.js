...
XPCOMUtils.defineLazyModuleGetter(this, "EwsNotification",
...
XPCOMUtils.defineLazyModuleGetter(this, "JaBaseIncomingServer",
...
function EwsIncomingServer(aDelegator, aBaseInterfaces) {
  ...
  // instance variables
  this._mailbox = null;
  this._unavailable = Ci.msqIEwsIncomingServer.UNCHECKED;
...
// Add an additional interface only needed by this custom class.
  extraInterfaces: [ Ci.msqIEwsIncomingServer ],
}
...
EwsIncomingServer.prototype = {
  ...
  performBiff(aMsgWindow) {
    Task.spawn( function* () {
      ...
      let newMessagesResult = yield promiseNewMessages.promise;
    ...
    // msqIEwsIncomingServer implementation
  ...
  get nativeMailbox() {
    if (!this._mailbox)
    {
     ...
     this.setupSoapLogging(this._mailbox);
      ...
      let activityListener = Cc["@mesquilla.com/ewsactivitylistener;1"]
                               .createInstance(Ci.msqIEwsActivityListener);
      this._mailbox.activityListener = activityListener;
  ...
  performExpandAsync(aMsgWindow, aUrlListener) {
    ...
    if (this.unavailable != Ci.msqIEwsIncomingServer.AVAILABLE)
    {
  ...
  createSubfolderWithListener(urlListener, parentMsgFolder, name)
  {
    Task.spawn(function* ()
    {
      ...
      let mailbox = this.nativeMailbox;
      let parentEwsFolder = safeGetInterface(parentMsgFolder, Ci.msqIEwsMailFolder);
      ...
      let childEwsFolder = safeGetInterface(childMsgFolder, Ci.msqIEwsMailFolder);
      ...
      if (!isJunkFolder)
      {
        ...
        let createResult = yield listener.promise;
  ...
  getAllAttachments(aMessage, aHeaderSink) {
    Task.spawn(function* ()
    {
      ...
      let ewsFolder = safeGetInterface(aMessage.folder, Ci.msqIEwsMailFolder);
      ...
      for (let attachmentIndex = 0; hasAttachments && attachmentIndex < length; attachmentIndex++)
      {
        ...
        let attachmentResult = yield promiseAttachment.promise;
  ...
  deleteSubfolders(aFolderIds)
  {
    ...
    Task.spawn( function* ()
    {
      ...
      let deleteResult = yield promiseDelete.promise;
  ...
  async saveAttachment(aContentType, aURL, aDisplayName, aMessageUri, aIsExternalAttachment) {
    ...
    let ewsUrl = safeGetInterface(uri, Ci.msqIEwsUrl);
...
function ewsDiscoverFoldersListener(aServer, aUrlListener) {
  ...
  Task.spawn(function* ()
  {
    ...
    if (aServer.unavailable == Ci.msqIEwsIncomingServer.AVAILABLE)
    ...
    let discoverResult = yield promiseDiscover.promise;
    ...
    while (directoriesEnum.hasMoreElements()) {
      ...
      let ewsDirectoryTest = safeGetInterface(directory, Ci.msqIEwsAbDirectory);
      if (!ewsDirectoryTest)
        continue;

      let nativeFolder = ewsDirectoryTest.nativeFolder;
      if ( !ewsDirectoryTest.isGAL && 
            (!ewsDirectoryTest.serverURI ||
             !(nativeFolder.folderId || nativeFolder.distinguishedFolderId) ||
             nativeFolder.hidden ||
             !nativeFolder.folderClass.startsWith("IPF.Contact")))
      {
        log.debug("ewsDirectoryTest.serverURI: " + ewsDirectoryTest.serverURI);
      ...
      if (aServer.serverURI == directory.serverURI)
    ...
    for (let directory of ewsDirectories) {
      let ewsDirectory = safeGetInterface(directory, Ci.msqIEwsAbDirectory);
      ...
      let updateAbResult = yield promiseUpdateAb.promise;
...
function ewsCheckOnlineListener(aServer)
{
  Task.spawn(function* ()
  {
    ...
    let rOnline = yield promiseOnline.promise;

    if (rOnline.status != Cr.NS_OK) {
      log.config("check online failed, setting offline");
      aServer.unavailable = Ci.msqIEwsIncomingServer.OFFLINE;
    ...
    aServer.unavailable = Ci.msqIEwsIncomingServer.AVAILABLE;
    let promiseExpand = new PromiseUtils.UrlListener();
    aServer.performExpandAsync(null, promiseExpand);
    let exitCode = yield promiseExpand.promise;

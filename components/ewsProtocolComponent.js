...
EwsProtocol.prototype = {
  ...
  QueryInterface: XPCOMUtils.generateQI(EwsProtocol.Properties.baseInterfaces),
  ...
  classInfo:      XPCOMUtils.generateCI({
                    ...
                    interfaces: EwsProtocol.Properties.baseInterfaces
               }),
  ...
  onEvent(aItem, aEvent, aData, aResult)
  {
    ...
    if (aEvent == ("MachineError"))
    {
      ...
      let ewsServer = safeGetInterface(mailnewsUrl.server, Ci.msqIEwsIncomingServer);
      ...
      do // error handling loop
      {
        ...
        if (!properties)
        {
          ...
          if (!folderId)
          {
            ...
            let ewsFolder = safeGetInterface(mailnewsUrl.folder, Ci.msqIEwsMailFolder);
            if (!ewsFolder) {
          ...
          if (folderId)
          {
            this._item.raiseFlags(Ci.msqIEwsNativeItem.Dirty);
            this._item.persist();
    ...
    else if (aEvent == ("StopMachine") || aEvent == ("FakeStopMachine"))
    {
      do {
        ...
        switch (this._state)
        {
          case PT_WAIT_BODY:
          {
            ...
            let outputHtml = (flags & Ci.msqIEwsNativeItem.BodyIsHtml);
            if (outputHtml)
            {
                ...
                this._protocolType == EPT_FILTER)
            {
              if (this._protocolType == EPT_FILTER)
              {
                ...
                let length = headersArray ? headersArray.length : 0;
                for (let i = 0; i < length; i++)
                {
                  let headerPL = headersArray.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject;
            ...
            this.onDataAvailable(this, null, inputStream, 0, inputStream.available());
            inputStream.close();
            break;
          }
          ...
          {
            let attachment = safeQueryInterface(aData, Ci.msqIEwsNativeAttachment);
            if (!attachment) {
            ...
            this.onDataAvailable(this, null, fstream, 0, fstream.available());
            fstream.close();
      ...
      if (!haveStartRequest)
        this.onStartRequest(this, this.channelContext);
    ...
    if (!haveStartRequest)
      this.onStartRequest(this, this._channelContext);
    this.onStopRequest(this, this._channelContext,
                       resultText ? Cr.NS_ERROR_FAILURE : Cr.NS_OK);
    return;
  ...
  onDataAvailable(aRequest, aContext, aInputStream, aOffset, aCount) {
    if (this._channelListener)
      this._channelListener.onDataAvailable(aRequest, aContext, aInputStream, aOffset, aCount);
  },
  ...
  {
    if (aContext instanceof Ci.nsIMsgMailNewsUrl) {
      aContext.SetUrlState(true, Cr.NS_OK);
      if (this._loadGroup)
    ...
    if (this._channelListener) {
      try { // we are seeing this throw, trying to understand.
        this._channelListener.onStartRequest(aRequest, aContext);
      } catch (e) {
  ...
  {
    if (this._channelListener)
      this._channelListener.onStopRequest(aRequest, aContext, aStatusCode);
    if (aContext instanceof Ci.nsIMsgMailNewsUrl)
    {
      aContext.SetUrlState(false, aStatusCode);
      if (this._loadGroup)
        this._loadGroup.removeRequest(this, null, aStatusCode);
    }
    else
      dl("aContext not nsIMsgMailNewsUrl");
  },
  ...
  asyncOpen2(aListener) {
    ...
    let listener = contentSecManager.performSecurityCheck(this, aListener);
...
// void asyncOpen (in nsIStreamListener aListener, in nsISupports aContext);
  asyncOpen(aListener, aContext) {
    ...
    let ewsServer = safeGetInterface(url.server, Ci.msqIEwsIncomingServer);
    ...
    let ewsUrl = safeGetInterface(url, Ci.msqIEwsUrl);
    if (!ewsUrl)
  ...
  if (!(this.protocolType  == EPT_EWSATTACHMENT)
     ...
     )
    {
      ...
      let conversionListener =
        streamConverter.asyncConvertData("message/rfc822", "*/*", aListener, this);
      this.setChannelListener(conversionListener);
    }
    else
    {
      this.setChannelListener(aListener);
    }
    ...
    if (aContext)
      this._channelContext = aContext;
    if (!this._channelContext)
      this._channelContext = this.URI;
  ...
  _rewriteCid(aBody, aItem)
  {
    ...
    if (changed)
    {
      ...
      // html serializer
      let serializer = Cc["@mozilla.org/layout/documentEncoder;1?type=text/html"]

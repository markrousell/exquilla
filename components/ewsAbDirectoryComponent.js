...
XPCOMUtils.defineLazyModuleGetter(this, "JaBaseAbDirectory",
...
EwsAbDirectory.Properties = {
  ...
  extraInterfaces: [ Ci.msqIEwsAbDirectory,
...
EwsAbDirectory.prototype = {
  ...
  { try {
    ...
    {
      this.mailbox.getNewItems(this.nativeFolder, safeGetInterface(this, Ci.msqIEwsEventListener));
  ...
  try {
    ...
    //nativeItem.raiseFlags(Ci.msqIEwsNativeItem.UpdatedLocally);
    if (didChange)
    {
  ...
  { try {
    ...
    for (let i = 0; emails && i < emails.length; i++)
    {
      let email = emails.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject;
      let value;
      try {
    ...
    for (let i = 0; addresses && i < addresses.length; i++)
    {
      let address = addresses.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject;
    ...
    for (let i = 0; phones && i < phones.length; i++)
    {
      let phone = phones.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject;
    ...
    if (aItem.flags & Ci.msqIEwsNativeItem.AllOnServer && !this.mIsGAL)
    {
      aItem.clearFlags(Ci.msqIEwsNativeItem.AllOnServer);
      aItem.persist();
    }
  ...
  updateDistList: function _updateDistList(aItem)
  {
    ...
    if (aItem.dlExpansion)
    {
      ...
      for (let i = 0; mailboxes && i < mailboxes.length; i++)
      {
        // todo: support complex dist list items
        let itemMailbox = mailboxes.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject;
        try {
  ...
  { try {
    if (!(safeInstanceOf(resolution, Ci.msqIPropertyList)))
    {
    ...
    item.flags = Ci.msqIEwsNativeItem.HasTempId;
  ...
  // msqIEwsAbDirectory implementation
        ...
        return this.wrap.mailbox.allIds(this.wrap.folderId,
            {
              onEvent: function _onEvent(aItem, aEvent, aData, result)
              {
                if (aEvent == "StopMachine")
                {
                  ...
                  let idArray = null;
                  try {
                    idArray = aData.QueryInterface(Ci.msqIStringArray).wrappedJSObject;
                  ...
                  if (idArray)
                  {
                    ...
                    for (let i = 0; i < count; i++)
                    {
                      ...
                      else
                      {
                        ...
                        item.raiseFlags(Ci.msqIEwsNativeItem.Dirty);
                      }
                    }
                  }
                  if (aListener)
  ...
  { try {
    if (aEvent == "StopMachine")
    {
      ...
      else if (this.wrap.resolvingNames)
      {
        ...
        try {
          resolutions = aData.QueryInterface(Ci.nsIArray);
        } catch (e) {}
        ...
        {
          let resolution = resolutions.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject;
          if (!resolution)
    ...
    {
      if (safeInstanceOf(aData, Ci.msqIEwsNativeItem))
...
{ try {
  if (aEvent == "StopMachine")
  {
    ...
    if (!resolutions || !resolutions.length)
      log.debug("resolveNames returned no results");
    for (let i = 0; resolutions && i < resolutions.length; i++)
    {
      let resolution = resolutions.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject;
      if (!resolution)
        log.warning("resolution not a property list")

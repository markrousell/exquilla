...
(function _exquilla()
{
  ...
  if (typeof (pub.Utils) == "undefined")
    Cu.import("resource://exquilla/ewsUtils.jsm", pub);
  Cu.import("resource://exquilla/ewsAbService.jsm", pub);
  Cu.import("resource://exquilla/PromiseUtils.jsm", pub);
  // local shorthands
  let re = pub.Utils.re;
  let dl = pub.Utils.dl;
  let log = pub.Utils.ewsLog;
  let safeGetInterface = pub.Utils.safeGetInterface;
  ...
  let serverListener = {
    onServerLoaded: function onServerLoaded(aServer) {
      if (safeGetInterface(aServer, Ci.msqIEwsIncomingServer))
      {
  ...
  function onLoad()
  {
    ...
    AddonManager.getAddonByID("exquilla@mesquilla.com", function(aAddon) {
      log.config("ExQuilla version is " + aAddon.version);
    });
    ...
    function findLoadedServer(aUsername, aHostname)
    {
      ...
      { try {
               ...
               ("exquilla" == rootBranch.getCharPref("mail.server." + server + ".type") ) )
          {
            ...
            if (account && safeGetInterface(account.incomingServer, Ci.msqIEwsIncomingServer))
              return true;
          }
        } catch (e) {}
      }
    ...
    {
      if (safeGetInterface(account.incomingServer, Ci.msqIEwsIncomingServer))
      { try {
          ...
          exquilla.Utils.manageNtlmUri(
            safeGetInterface(account.incomingServer, Ci.msqIEwsIncomingServer).ewsURL, true);
    ...
    try  {
      Cu.import("resource://exquilla/ewsActivity.js");
    ...
    gFolderTreeView._sortedAccounts = function _ewsSortedAccounts()
    {
      return exquilla.oldSortedAccounts().filter(
        ...
        {
          let ewsServer = safeGetInterface(a.incomingServer, Ci.msqIEwsIncomingServer);
      ...
      do {
        try {
          ...
          if (!(folders[0].flags & Ci.nsMsgFolderFlags.Queue))
            break;
          if (!(safeGetInterface(folders[0], Ci.msqIEwsMailFolder)))
            break;
    ...
    SendUnsentMessages = function ()
    {
      try {
        let folders = gFolderTreeView.getSelectedFolders();
        if (folders.length &&
            safeGetInterface(folders[0], Ci.msqIEwsMailFolder) &&
    ...
    function ewsSendUnsentMessages(aFolder)
    {
      ...
      let ewsFolder = safeGetInterface(aFolder, Ci.msqIEwsMailFolder);
      if (identity.doFcc)
      {
    ...
    */
    ...
    Cu.import("resource:///modules/gloda/index_msg.js", exquilla);
    Cu.import("resource:///modules/gloda/datastore.js", exquilla);

    function ewsShouldIndexFolder(aMsgFolder)
    {
      let ewsFolder = safeGetInterface(aMsgFolder, Ci.msqIEwsMailFolder);
      if (!ewsFolder)
    ...
    gFolderDisplay.doCommand = function _doCommand(aCommandName)
    {
      ...
      if (safeGetInterface(folder, Ci.msqIEwsMailFolder))
      {
        switch (aCommandName)
        {
  ...
  {
    // http://exquillafirstrun.mesquilla.com typically redirects to
    // http://exquilla.zendesk.com/entries/20887131-after-installation-creating-an-exchange-account
    let handlerRegExp = "^https?://(?:(?:exquilla\.zendesk\.com)|(?:mesquilla\.com))/";
    exquilla.Utils.openContentTab("http://exquillafirstrun.mesquilla.com", handlerRegExp, 1000);
  }
  ...
  function observe(aSubject, aTopic, aData)
  {
    ...
    {
      log.debug('observed mail-startup-done');
      exquilla.EwsAbService.loadEwsServers();
    }
  }

...
XPCOMUtils.defineLazyModuleGetter(this, "EWStoPL", "resource://exquilla/EWStoPL.js");
...
// object constructors
//const PropertyList =
//  Components.Constructor("@mesquilla.com/webservices/propertylist;1",
//                         Ci.msqIPropertyList);
...
// @parm aNativeFolders: array of msqIEwsNativeFolder objects
...
// @parm msqIEwsNativeFolder folder, the folder to update
// @parm nsIDOMElement element, a t:Folder element
...
function updateParentSubfolders(folder)
{
  ...
  if (maybeFolder)
  {
    ...
    if (maybeIndex != Ci.msqIStringArray.noIndex)
      subfolderIds.removeAt(maybeIndex);
  }

  if (subfolderIds.indexOf(folder.folderId) == Ci.msqIStringArray.noIndex) {
    ...
    if (this._messageString)
      this._message =
        Cc["@mozilla.org/xmlextras/domparser;1"]
          ...
          .parseFromString(this._messageString, "text/xml");
    return this._message;
  },
...
EwsSoapRequest.Properties = {

  // Interfaces implemented by the base CPP version of this object.
  baseInterfaces:     [ Ci.msqIEwsSoapRequest,
                        Ci.msqISOAPResponseListener,
                        Ci.nsISupports,
                        Ci.nsIInterfaceRequestor,
                        ],

  classID:            Components.ID("{C4E20960-304A-42F5-B79E-BFBB4C6C3F2B}"),
  classDescription:   "ExQuilla Soap Request",
};

EwsSoapRequest.prototype = {
  // QI to the interfaces.
  QueryInterface: XPCOMUtils.generateQI(EwsSoapRequest.Properties.baseInterfaces),
  classID: Components.ID("{C4E20960-304A-42F5-B79E-BFBB4C6C3F2B}"),
  classInfo: XPCOMUtils.generateCI({
                 contractID: null,
                 classDescription: null,
                 flags: Ci.nsIClassInfo.SINGLETON,
                 interfaces: EwsSoapRequest.Properties.baseInterfaces
               }),
  ...
  getFolders(aResponse, aNativeFolders)
  {
    ...
    let folders = [];
    for (let i = 0; i < aNativeFolders.length; i++) {
      let folder = aNativeFolders.queryElementAt(i, Ci.msqIEwsNativeFolder);
      folders.push(folder);
    }
    let body = `<m:GetFolder>${folderShapeXML()}` +
               `${(folderIdsXML(folders))}</m:GetFolder>`;
  ...
  createFolders(aResponse, aParentFolder, aChildFolders)
  {
    ...
    let body =
      `<m:CreateFolder>` +
        `<m:ParentFolderId>${folderIdXML(aParentFolder)}</m:ParentFolderId>` +
        `<m:Folders>`;
    for (let i = 0; i < aChildFolders.length; i++) {
      let childFolder = aChildFolders.queryElementAt(i, Ci.msqIEwsNativeFolder);
  ...
  deleteFolders(aResponse, aFolderIds, aMoveToDeletedItems)
  {
    ...
    this._ids = aFolderIds;  // an msqIStringArray
    this._nextFolderIndex = 0;
    let folders = [];
...
*/
                            ...
                            aMaxChangesReturned)
  {
    ...
    this._nativeFolder = aNativeFolder;
    this._items = aItems; // changed items will be stored here as msqIEwsNativeItem
    let body =
      `<m:SyncFolderItems>` +
        `<m:ItemShape>` +
  ...
  getItemBodies(aResponse, aNativeItems)
  {
    ...
    for (let i = 0; i < aNativeItems.length; i++) {
      let item = aNativeItems.queryElementAt(i, Ci.msqIEwsNativeItem);
...
*/
  subscribeNotifications(aResponse, aNativeFolders, aCheckAll) {
    ...
    if (!aCheckAll) {
      for (let i = 0; i < aNativeFolders.length; i++) {
        let nativeFolder = aNativeFolders.queryElementAt(i, Ci.msqIEwsNativeFolder);
        folderIdsXML += folderIdXML(nativeFolder);
      }
    }
    ...
    let body = 
      `<m:Subscribe>` +
        `<m:StreamingSubscriptionRequest SubscribeToAllFolders="${aCheckAll ? 'true' : 'false'}">` +
          (folderIdsXML ? `<FolderIds>` + folderIdsXML + `</FolderIds>` : "") +
          `<EventTypes>` +
...
*/
  ...
  saveManyUpdates(aResponse, aNativeItems) {
    ...
    for (let i = 0; i < aNativeItems.length; i++) {
      ...
      if (item.flags & Ci.msqIEwsNativeItem.DeletedOnServerBit)
      {
...
*/
  ...
  invoke()
  {
    ...
    //           onStartRequest
    if (this.ewsResponse) {
      this.ewsResponse.onStartRequest(this, null);
    }
    else {
    ...
    if (rv != Cr.NS_OK) {
      ...
      if (this.ewsResponse)
      {
        this.ewsResponse.onStopRequest(this, null, rv);
  ...
  // msqISOAPResponseListener overrides
  handleResponse(/* in msqISOAPResponse */   aResponse,
                 /* in msqISOAPCall aCall */ aCall,
                 /* in nsresult */           aStatus,
                 /* in boolean */            aLast)
  {
    ...
    if (!aLast) {
      ...
      do { // break when error or done
        ...
        let elementDOM =
          Cc["@mozilla.org/xmlextras/domparser;1"]
            .createInstance(Ci.nsIDOMParser)
            .parseFromString(elementText, "text/xml");
        ...
        for (let i = 0; i < streamingResponsePL.length; i++) {
          ...
          if (responseMessage) {
            ...
            for (let j = 0; j < notifications.length; j++) {
              let notification = notifications.queryElementAt(j, Ci.msqIPropertyList);
    ...
    if (this.ewsResponse) {
      this.ewsResponse.onStopRequest(this, null,
           haveFault ? errorReturn : Cr.NS_OK);
      this.ewsResponse = null;
    }
    if (this.mailbox) {
      this.mailbox.finishRequest(this);
  ...
  _error(aResponseError, aResponseCode, aMessageText)
  {
    ...
    if (this.ewsResponse) {
      this.ewsResponse.errorResponse(this, null,
  ...
  _FolderInfoResponseType(response) {
    ...
    for (let folderElement of foldersElement.children) { // <t:Folder>
      ...
      if (this.requestName == "MoveFolders") {
        ...
        nativeFolder = this._nativeFolders
          .queryElementAt(this._nextFolderIndex++, Ci.msqIEwsNativeFolder);
      }
      let folderIdElement =
        folderElement.getElementsByTagNameNS(nsTypes, "FolderId")[0];
      let folderId = folderIdElement.getAttribute("Id");
  ...
  _SyncFolderItemsType(response)
  {
    ...
    for (let changeElement of changeElements) {
      ...
      if (changeType == "Create") {
        ...
        changedItem.raiseFlags(Ci.msqIEwsNativeItem.DeletedOnServerBit);
      ...
      if (didNotChange) {
        changedItem.processingFlags |= Ci.msqIEwsNativeItem.DidNotChange;
      }
      else {
        changedItem.processingFlags &= ~Ci.msqIEwsNativeItem.DidNotChange;
      }
    ...
    if (responseClass == "Error") {
      ...
      if (this.requestName == "GetChangedItemProperties") {
        ...
        if (responseCode == "ErrorItemNotFound") {
          nativeItem.raiseFlags(Ci.msqIEwsNativeItem.DeletedOnServerBit);
          return true;
        }
      }
      return false;
    ...
    try {
      ...
      for (let itemElement of itemElements) {
        ...
        if (this.requestName == "GetItemBodies") {
          ...
          if (bodyType == "HTML") {
            nativeItem.flags |= Ci.msqIEwsNativeItem.BodyIsHtml;
          }
          else {
            nativeItem.flags &= ~Ci.msqIEwsNativeItem.BodyIsHtml;
          }
          ...
          nativeItem.clearFlags(Ci.msqIEwsNativeItem.HasTempId |
                                Ci.msqIEwsNativeItem.NewLocally);
          ...
          if (newItems.length >= currentIndex + 1) {
            try {
              newItem = newItems.queryElementAt(currentIndex, Ci.msqIEwsNativeItem);
            }
            catch (e) {
          ...
          newItem.clearFlags(Ci.msqIEwsNativeItem.DeletedBit |
                             Ci.msqIEwsNativeItem.AllLocally |
          ...
          newItem.raiseFlags(Ci.msqIEwsNativeItem.NeedsResync);

          if (this._isMove)
            oldItem.raiseFlags(Ci.msqIEwsNativeItem.DeletedOnServerBit);
        }
          ...
          if (this._getBody)
          {
            ...
            if (bodyType == "HTML")
              flags |= Ci.msqIEwsNativeItem.BodyIsHtml;
            else
              flags &= ~Ci.msqIEwsNativeItem.BodyIsHtml;
          ...
          if (hasBody)
            nativeItem.processingFlags |= Ci.msqIEwsNativeItem.HasBody;
          else
            nativeItem.processingFlags &= ~Ci.msqIEwsNativeItem.HasBody;

          nativeItem.properties = Item_PL;
          nativeItem.clearFlags(Ci.msqIEwsNativeItem.DeletedLocally
          ...
          let item = this._items.queryElementAt(this._nextIdIndex++, Ci.msqIEwsNativeItem);
          if ( (item.flags & Ci.msqIEwsNativeItem.HasTempId) ||
               !(item.itemId == itemId)) {
            this.mailbox.changeItemId(item, itemId);
          }
          item.changeKey = changeKey;
          item.clearFlags(Ci.msqIEwsNativeItem.AllLocally |
                          Ci.msqIEwsNativeItem.HasTempId);
...
function stripText(aElement, aName, aNamespace)
{
  ...
  for (let i = 0; i < nodes.length; i++)
  {
    ...
    if (node)
    {
      ...
      {
        if (child instanceof Ci.nsIDOMCharacterData)
          child.data = "...hidden...";
      }
    }
  }
...
var NSGetFactory = XPCOMUtils.generateNSGetFactory([EwsSoapRequest]);

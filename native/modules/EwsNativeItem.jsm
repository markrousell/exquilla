...
XPCOMUtils.defineLazyModuleGetter(this, "EWStoPL",
...
// Convert a properties string parsed into an nsIDOMElement from one server type to another
...
EwsNativeItem.prototype = {
  ...
  QueryInterface:   XPCOMUtils.generateQI([Ci.msqIEwsNativeItem]),
  classInfo:        XPCOMUtils.generateCI({
                     classID: Components.ID("{BE4E14E9-308D-41E2-92D7-E4F920776684}"),
                     classDescription: "ExQuilla native item",
                     contractID: "@mesquilla.com/ewsnativeitem;1",
                     interfaces: [Ci.msqIEwsNativeItem],
                     flags: 0,
                   }),
  ...
  get exItemId() {
        ...
        this._originalStart.length > 0)
    {
      ...
      else
      {
        ...
        //mFlags |= msqIEwsNativeItem::HasTempId;
  ...
  get updatedOnServer() {
    return !!(this._flags & Ci.msqIEwsNativeItem.UpdatedOnServerBit);
  },
  set updatedOnServer(value) {
    if (value) {
      this.raiseFlags(Ci.msqIEwsNativeItem.UpdatedOnServerBit);
      this.clearFlags(Ci.msqIEwsNativeItem.NewOnServerBit |
                      Ci.msqIEwsNativeItem.DeletedOnServerBit);
    }
    else
      this.clearFlags(Ci.msqIEwsNativeItem.UpdatedOnServerBit);
  },

  /// item is newly created on server
  //attribute boolean newOnServer;
  get newOnServer() {
    return !!(this._flags & Ci.msqIEwsNativeItem.NewOnServerBit);
  },
  set newOnServer(value) {
    if (value) {
      this.raiseFlags(Ci.msqIEwsNativeItem.NewOnServerBit);
      this.clearFlags(Ci.msqIEwsNativeItem.UpdatedOnServerBit |
                      Ci.msqIEwsNativeItem.DeletedOnServerBit);
    }
    else
  ...
  get deletedOnServer() {
    return !!(this._flags & Ci.msqIEwsNativeItem.DeletedOnServerBit);
  },
  set deletedOnServer(value) {
    if (value) {
      this.raiseFlags(Ci.msqIEwsNativeItem.DeletedOnServerBit);
      this.clearFlags(Ci.msqIEwsNativeItem.NewOnServerBit |
                      Ci.msqIEwsNativeItem.UpdatedOnServerBit);
    }
    else
      this.clearFlags(Ci.msqIEwsNativeItem.DeletedOnServerBit);
  },
  ...
  get needsProperties() { return !!(this._flags & Ci.msqIEwsNativeItem.NeedsPropertiesBit);},
  set needsProperties(aValue) {
    if (aValue)
      this.raiseFlags(Ci.msqIEwsNativeItem.NeedsPropertiesBit);
    else
      this.clearFlags(Ci.msqIEwsNativeItem.NeedsPropertiesBit);
  },

  /// item is marked as deleted on the server
  //attribute boolean deleted;
  get deleted() { return !!(this._flags & Ci.msqIEwsNativeItem.DeletedBit);},
  set deleted(aValue) {
    if (aValue)
      this.raiseFlags(Ci.msqIEwsNativeItem.DeletedBit);
    else
      this.clearFlags(Ci.msqIEwsNativeItem.DeletedBit);
  },
  ...
  set propertiesString(aString) {
    ...
    let parsedElement = Cc["@mozilla.org/xmlextras/domparser;1"]
                          .createInstance(Ci.nsIDOMParser)
                          .parseFromString(aString, "text/xml")
                          .documentElement;
  ...
  set body(value) {
    this._body = value;
    if (value)
      this._processingFlags |= Ci.msqIEwsNativeItem.HasBody;
    else
      this._processingFlags &= ~Ci.msqIEwsNativeItem.HasBody;
  },
  ...
  clone: function (aItemId, aChangeKey, aNativeFolder) {
    ...
    let flags = this._flags & ~Ci.msqIEwsNativeItem.HasOfflineBody;
    let itemId = aItemId;
    if (!itemId) {
      itemId = Cc["@mozilla.org/uuid-generator;1"]
                 ...
                 .generateUUID()
                 .toString();
      flags |= Ci.msqIEwsNativeItem.HasTempId;
    ...
    newItem.processingFlags = this._processingFlags & ~Ci.msqIEwsNativeItem.HasBody;
    ...
    do { // break to continue without delete.
      ...
      if (mailbox.attachmentsDirectory.contains(file)) {
        ...
        try {
          ...
          log.error("Could not remove local attachment file, but continuing");
        }
      }
    } while (false);
  ...
  //void mergeChanges(in msqIPropertyList newProperties);
  mergeChanges: function(newProperties) {
    ...
    this._flags |= Ci.msqIEwsNativeItem.UpdatedLocally;
  ...
  setExtendedProperty: function(aPropertyTag, aPropertyType, aValue, aProperties) {
    ...
    if (extendedProperties) {
      for (let i = 0; i < extendedProperties.length; i++) {
        plExtendedProperty = extendedProperties.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject;
  ...
  _getExtendedPropertyValue: function(aPropertyTag, aProperties) {
    ...
    if (extendedProperties) {
      for (let i = 0; i < extendedProperties.length; i++) {
        let plExtendedProperty = extendedProperties.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject;
        if (plExtendedProperty) {
          let propertyTag = plExtendedProperty.getAString("ExtendedFieldURI/$attributes/PropertyTag");
          if (propertyTag == aPropertyTag)

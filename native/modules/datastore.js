...
{ try {
  ...
  // context types
  if (aContext instanceof Ci.nsISupports) {
    this.context = aContext;
    if (safeInstanceOf(aContext, Ci.msqIEwsNativeItem))
      this.item = aContext.wrappedJSObject;
    else if (safeGetInterface(aContext, Ci.msqIEwsNativeFolder))
      this.folder = aContext.wrappedJSObject;
  }
  ...
  if (this.type == 'putBody')
  {
    if (this.item)
    {
      if (aReason == Ci.mozIStorageStatementCallback.REASON_FINISHED)
      {
        ...
        this.item.flags |= Ci.msqIEwsNativeItem.HasOfflineBody;
      }
...
{ try {
  ...
  if (this.type == 'getItem')
  {
    ...
    this.item.flags = row.getResultByName('flags') | Ci.msqIEwsNativeItem.Persisted;
...
{
  QueryInterface:   XPCOMUtils.generateQI([Components.interfaces.msqIEwsDatastore]),
  classInfo:        XPCOMUtils.generateCI({classID: null,
                                           contractID: null,
                                           classDescription: "ExQuilla Datastore",
                                           interfaces: [Ci.msqIEwsDatastore]}),
  ...
  open: function eds_open(aDbDirectory) {
    ...
    this._dbStatements.push(this._deleteFolderDataStatement = this.dbConnection.createStatement(
      "UPDATE itemMetadata SET flags = (flags | 65536) WHERE folderId = :folderId")); // msqIEwsNativeItem.NeedsResync
   ...
   *
   * @param aNativeItem  msqIEwsNativeItem to be saved
   * @param aListener    msqIEwsEventListener to receive notifications
   */
  ...
  { try {
    ...
    // sync metadata db
    aNativeItem.flags |= Ci.msqIEwsNativeItem.Persisted;
    ...
    if (!properties.length)
    {
      ...
      if (!(aNativeItem.flags & Ci.msqIEwsNativeItem.Dirty) &&
          !(aNativeItem.flags & Ci.msqIEwsNativeItem.NeedsResync))
      {
    ...
    let changedOnServer = aNativeItem.flags & (
        ...
        Ci.msqIEwsNativeItem.DeletedBit |
    ...
    else
    {
      ...
      try {
        while (statement.step()) {
          ...
          aNativeItem.flags = (statement.row.flags) | Ci.msqIEwsNativeItem.Persisted;
  ...
  putBody: function eds_putBody(aNativeItem, aListener)
  {
    ...
    else
    {
      ...
      if (setBody)
        aNativeItem.flags |= Ci.msqIEwsNativeItem.HasOfflineBody;
      else
        aNativeItem.flags &= ~Ci.msqIEwsNativeItem.HasOfflineBody;
    }
  },
  ...
  { try {
    ...
    return connection;
  } catch(e) {re("Could not connect to database file " + aFileName + '\n' + e);}},

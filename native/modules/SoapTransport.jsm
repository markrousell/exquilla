...
XPCOMUtils.defineLazyModuleGetter(this, "EwsFBA",
  "resource://exquilla/EwsFBA.jsm");
XPCOMUtils.defineLazyModuleGetter(this, "MonitoredRequest",
...
var oSerializer = Cc["@mozilla.org/xmlextras/xmlserializer;1"]
...
const SoapTransportInterfaces = [Ci.msqISOAPTransport, Ci.nsISupports, Ci.nsIClassInfo, Ci.nsIAuthPrompt2,
...
SoapTransport.prototype =
{
  ...
  */
  ...
  flags:            0,
  QueryInterface:   XPCOMUtils.generateQI(SoapTransportInterfaces),

  // msqISOAPTransport implementation
   ...
   *
   * @param aCall (msqISOAPCall): Actual message to be sent.
   *
   * @param aListener (msqISOAPResponseListener) Handler to be invoked (single threaded) as each response is
   *  received and finally with null.  If specified as null, no responses are returned.
   *
   * @param response (msqISOAPResponse) Message to receive response and be handled by listener.  May be
...
{
  QueryInterface: XPCOMUtils.generateQI([Ci.nsIDOMEventListener, Ci.msqISOAPCallCompletion, Ci.nsISupports]),
  classInfo:        XPCOMUtils.generateCI({classID: null,
                                           contractID: null,
                                           classDescription: "Soap Transport Completion",
                                           interfaces: [Ci.nsIDOMEventListener,
                                                        Ci.msqISOAPCallCompletion,
                                                        Ci.nsISupports]}),

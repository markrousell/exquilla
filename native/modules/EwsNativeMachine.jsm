...
EwsNativeMachine.prototype = {
  ...
  QueryInterface:   XPCOMUtils.generateQI([Components.interfaces.msqIEwsMachine,
                                           Components.interfaces.msqIEwsEventListener]),
  classInfo:        XPCOMUtils.generateCI({classID: null,
                                           contractID: null,
                                           classDescription: "ExQuilla Native Machine",
                                           interfaces: [Ci.msqIEwsEventListener, Ci.msqIEwsMachine]}),
  ...
  genericMachineDoTask: function _GenericMachineDoTask(aInnerTask, aMailbox, aListener) {
    ...
    Task.spawn(
      function* _taskGenericMachine() {
        try {
          if (aListener) aListener.onEvent(this, "StartMachine", null, Cr.NS_OK);
          result = yield promiseInitial(aMailbox, aListener);

          if (result.status == Cr.NS_OK && aInnerTask) {
            if (!isGenerator(aInnerTask))
              result = yield aInnerTask();
            else
              result = yield aInnerTask;
          }
        }
    ...
    .then(
      ...
      (e) => {
        this.machineErrorResponse(aListener, "GenericMachineReject", null, se(e));
  ...
  checkOnline: function _checkOnline(aMailbox, aListener)
  {
    Task.spawn(function* checkOnlineMachine() {
      if (aListener) aListener.onEvent(this, "StartMachine", null, Cr.NS_OK);
      let result = yield promiseCheckOnline(aMailbox, aListener);
      this.stopMachine(aListener, result);
    }.bind(this))
    .then(null, (e) => {
  ...
  discoverFolders: function _discoverFolders(aMailbox, aListener)
  {
    Task.spawn( function* taskDiscoverFolders() {
      let result = {status: Cr.NS_ERROR_FAILURE};
      try {
        if (aListener) aListener.onEvent(this, "StartMachine", null, Cr.NS_OK);
        do {
          ...
          {
            result = yield promiseCheckOnline(aMailbox, aListener);

            log.config('discoverFoldersMachine checkOnline result is ' + result.status);
          }
          if (!aMailbox.isOnline)
          {
            result = yield PromiseUtils.promiseAutodiscover(aMailbox.email.length ? aMailbox.email : aMailbox.username,
          ...
          result = yield promiseGetDistinguishedIds(aMailbox, aListener);
          ...
          result = yield promiseDiscoverSubfolders(aMailbox, rootFolder, aListener);
  ...
  createSubfolder: function _createSubfolder(aMailbox, aListener, aParentFolder, aChildFolder)
  {
    function* taskCreateSubfolder()
    {
      ...
      let result = yield soapResponse.promise;
      return (result);
    }
      ...
      try {
        ...
        else if (aClass == 1)
        {
          ...
          try {
            ...
            result = yield taskInvalid(aMailbox, aListener);
          }
          catch (e) {
            if (result.status == Cr.NS_OK)
              result.status = Cr.NS_ERROR_FAILURE;
  ...
  {
    function* taskGetItemBodyMachine() {
      log.debug("taskGetItemBodyMachine");
      let result = {status: Cr.NS_ERROR_FAILURE};
      try {
        do {
          if (aNativeItem.processingFlags & Ci.msqIEwsNativeItem.HasBody)
          {
          ...
          {
            aNativeItem.processingFlags |= Ci.msqIEwsNativeItem.HasBody;
            result.status = Cr.NS_OK;
            break;
          }

          // Do we have the body in storage?
          if (aNativeItem.flags & Ci.msqIEwsNativeItem.HasOfflineBody)
          {
            ...
            result = yield dsListener.promise;
            break;
          }
  ...
  {
    function* taskGetAttachmentContent()
    {
      let result = {status: Cr.NS_ERROR_FAILURE};
      try {
        do {
      ...
      try {
        do {
          ...
          for (let i = 0; i < aItemIds.length; i++)
          {
            ...
            if (aIsMove)
              item.raiseFlags(Ci.msqIEwsNativeItem.DeletedLocally);
            oldItems.push(item);
          }
          ...
          {
            let item = newItems.queryElementAt(i, Ci.msqIEwsNativeItem);
            item.clearFlags(Ci.msqIEwsNativeItem.AllOnServer);
            let dsListener = new PromiseUtils.DatastoreListener();
            ...
            if (!(item.processingFlags & Ci.msqIEwsNativeItem.HasBody))
            {
              // how about the old item?
              let oldItem = oldItems[i];
              if (oldItem.flags & Ci.msqIEwsNativeItem.HasOfflineBody &&
                  !(oldItem.processingFlags & Ci.msqIEwsNativeItem.HasBody)) {
                let dsListener = new PromiseUtils.DatastoreListener();
                aMailbox.datastore.getBody(item, dsListener);
                result = yield dsListener.promise;
                // ignore error
              }
              if (oldItem.processingFlags & Ci.msqIEwsNativeItem.HasBody)
          ...
          for (let oldItem of oldItems)
          {
            if (oldItem.deleted)
            {
              ...
              oldItem.processingFlags |= Ci.msqIEwsNativeItem.DeletedInDatastore;
...
//                        in msqIStringArray aFoldersIds, in boolean aMoveToDeletedItems);
      ...
      try {
        ...
        do {
          ...
          for (let i = 0; i < aItemIds.length; i++)
          {
            let item = aMailbox.getItem(aItemIds.getAt(i));
            item.raiseFlags(Ci.msqIEwsNativeItem.DeletedLocally);
            item.clearFlags(Ci.msqIEwsNativeItem.NewLocally | Ci.msqIEwsNativeItem.UpdatedLocally);
            let dsListener = new PromiseUtils.DatastoreListener();
            aMailbox.datastore.putItem(item, dsListener);
            result = yield dsListener.promise;
          }
          ...
          for (let i = 0; i < aItemIds.length; i++)
          {
            ...
            item.clearFlags(Ci.msqIEwsNativeItem.DeletedLocally);
            let dsListener = new PromiseUtils.DatastoreListener();
            aMailbox.datastore.putItem(item, dsListener);
            result = yield dsListener.promise;
          }
  ...
  {
    function* taskDiscoverSubfoldersCall()
    {
      let result = {status: Cr.NS_ERROR_FAILURE};
      try {
        result = yield promiseDiscoverSubfolders(aMailbox, aFolder, aListener);
      }
  ...
  allIds: function _allIds(aMailbox, aListener, aFolderId)
  {
    let result = {status: Cr.NS_ERROR_FAILURE};
    function* taskAllIds() {
      ...
      try {
        ...
        let changeKeys = result.item.QueryInterface(Ci.msqIStringArray).wrappedJSObject;
        ...
        {
          if (itemIds.indexOf(cachedIds.getAt(i)) == Ci.msqIStringArray.noIndex)
          {
            if (!gaveWarning)
            {
              gaveWarning = true;
    ...
    Task.spawn(taskAllIds.bind(this)).then(
      (result) => {
    ...
    function* taskGetItemOffline()
    {
      ...
      try {
        do {
          ...
          if (!(aItem.flags & Ci.msqIEwsNativeItem.HasOfflineBody))
          {
    ...
    function* taskCreateMessageFromString()
    {
      ...
      try {
        do {
          ...
          nativeItem.raiseFlags(Ci.msqIEwsNativeItem.NeedsResync);
          let dsListener = new PromiseUtils.DatastoreListener();
    ...
    function* taskCreateMessageFromFile()
    {
      ...
      try {
        do {
          let nativeItem = aMailbox.createItem("", "IPM.Note", aFolder);
          nativeItem.properties = aProperties;
          result = yield PromiseUtils.promiseAsyncFetch(aFile);
          if (!Components.isSuccessCode(result.status))
            break;
...
//                   in msqIEwsNativeItem aNativeItem);
      ...
      try {
        do {
          if (!aItem.itemId || !aItem.itemId.length)
          {
            ...
            aItem.itemId = uuid;
            aItem.raiseFlags(Ci.msqIEwsNativeItem.HasTempId);
          }

          // update flags
          aItem.raiseFlags(Ci.msqIEwsNativeItem.NewLocally);
          let dsListener = new PromiseUtils.DatastoreListener();
          aMailbox.datastore.putItem(aItem, dsListener);
          result = yield dsListener.promise;
          if (result.status != Cr.NS_OK)
          ...
          aItem.raiseFlags(Ci.msqIEwsNativeItem.NeedsResync);
  ...
  updateManyItems: function _updateManyItems(aMailbox, aListener, aItems)
  {
    function* taskUpdateManyItems()
    {
      ...
      try {
        ...
        {
          let item = aItems.queryElementAt(i, Ci.msqIEwsNativeItem);
          ...
          else
          {
            ...
            if (!(item.flags & Ci.msqIEwsNativeItem.HasTempId))
        ...
        {
          let item = itemsToUpdate.queryElementAt(ip1 - 1, Ci.msqIEwsNativeItem);
          if (item.flags & (Ci.msqIEwsNativeItem.DeletedBit |
                            Ci.msqIEwsNativeItem.DeletedOnServerBit ||
                            Ci.msqIEwsNativeItem.DeletedLocally))
  ...
  {
    function* taskUpdateItemProperties()
    {
      let result = {status: Cr.NS_ERROR_FAILURE};
      try {
        let rv = Cr.NS_OK;
        ...
        result = yield promiseDeletedOccurrences(aItem, aMailbox, aListener);
        if (result.status != Cr.NS_OK)
        ...
        //  updates PL
        try {
          aItem.localProperties.removeItem("Updates");
        } catch (e) {}
    ...
    function* taskGetNewItems()
    {
      ...
      try {
        ...
        try { // continue anyway if error
          ...
          let result = yield dsListener.promise;
          if (result.status != Cr.NS_OK)
            throw new CE("Failed to get items with missing folder id", Cr.NS_ERROR_FAILURE);
          let missingFolderIds = result.data.QueryInterface(Ci.msqIStringArray).wrappedJSObject;
          ...
          for (let i = 0; i < missingFolderIds.length; i++)
          {
            ...
            try {
              ...
              if ( (result.responseCode && result.responseCode == "ErrorItemNotFound") ||
                   ...
                   (changedItem.flags & Ci.msqIEwsNativeItem.DeletedOnServerBit)
              )
              {
                ...
                yield dsListener.promise;
              }
              else if (result.status == Cr.NS_OK)
              {
                changedItem.raiseFlags(Ci.msqIEwsNativeItem.Dirty);
                changedItem.clearFlags(Ci.msqIEwsNativeItem.AllLocally);
                let dsListener = new PromiseUtils.DatastoreListener();
            ...
            catch (e)
            {
              ...
              try {
                ...
                aMailbox.datastore.deleteItem(changedItem, dsListener);
                yield dsListener.promise;
              } catch (e) {log.error('delete of failed missing folder item failed: ' + e);}
            }
          }
        ...
        let changedLocallyIds = result.data.QueryInterface(Ci.msqIStringArray).wrappedJSObject;
        let dirtyIds = [];
        ...
        for (let i = 0; i < changedLocallyIds.length; i++)
        {
          ...
          try {
            ...
            if (flags & (Ci.msqIEwsNativeItem.Dirty | Ci.msqIEwsNativeItem.NeedsResync))
            {
            ...
            if (flags & Ci.msqIEwsNativeItem.DeletedLocally)
            {
            ...
            else if (flags & Ci.msqIEwsNativeItem.NewLocally)
            {
            ...
            else if (flags & Ci.msqIEwsNativeItem.UpdatedLocally)
            {
              ...
              else if (rv == Cr.NS_ERROR_ALREADY_INITIALIZED)
              {
                try {
                  ...
                  changedItem.clearFlags(Ci.msqIEwsNativeItem.UpdatedLocally);
                  changedItem.localProperties.removeItem("Updates");
                } catch (e) {}
              }
              else
                throw e;
            ...
            else
            {
              ...
              //
              if (changedItem.flags & Ci.msqIEwsNativeItem.DeletedBit) {
              ...
              else if (changedItem.flags & Ci.msqIEwsNativeItem.AllOnServer)
              {
                log.debug("No changed detected, clearing AllOnServer flags");
                changedItem.clearFlags(Ci.msqIEwsNativeItem.AllOnServer);
              }
            ...
            else if ( (result.status != Cr.NS_OK) && changedItem)
            {
              ...
              changedItem.raiseFlags(Ci.msqIEwsNativeItem.Dirty)
              changedItem.clearFlags(Ci.msqIEwsNativeItem.NewLocally ||
                                     Ci.msqIEwsNativeItem.DeletedLocally ||
                                     Ci.msqIEwsNativeItem.UpdatedLocally);
            ...
            else if (changedItem)
            {
              ...
              let result = yield promiseDeletedOccurrences(changedItem, aMailbox, aListener);
              if (result.status != Cr.NS_OK)
        ...
        for (let itemsPending = true; itemsPending;)
        {
          ...
          result = yield promiseChangedItems(aFolder, syncState, itemsPerSync);
          ...
          {
            let itemId = items.queryElementAt(i, Ci.msqIEwsNativeItem).itemId;
          ...
          // Divide the items into batches to process
            ...
            while(nextItemToGet < items.length)
            {
              ...
              {
                let changedItem = items.queryElementAt(nextItemToGet, Ci.msqIEwsNativeItem);
                ...
                let didNotChange = changedItem.processingFlags & Ci.msqIEwsNativeItem.DidNotChange;
                if (!somethingChanged && !didNotChange)
                  somethingChanged = true;

                // dirty items are handled one at a time
                if (changedItem.flags & Ci.msqIEwsNativeItem.Dirty)
                {
                     ...
                     (changedItem.deletedOnServer))
                {
                  ...
                  syncPromises.push(promiseItem.then(
                    result => {
                      if (safeInstanceOf(result.item, Ci.msqIEwsNativeItem))
                      {
                        if (aListener)
                ...
                try {
                  if (changedItem.flags & Ci.msqIEwsNativeItem.NewOnServerBit)
                  {
                    if (changedItem.flags & Ci.msqIEwsNativeItem.HasOfflineBody) {
                      let dsListener = new PromiseUtils.DatastoreListener();
                      ...
                      if (!changedItem.body.length) {
                        ...
                        changedItem.flags &= ~Ci.msqIEwsNativeItem.HasOfflineBody;
                      }
            ...
            for (let getChangedIds of batchIdLists)
            {
              ...
              syncPromises.push(Task.spawn( function* _taskGetIds() {
                if (aMailbox.isShutdown)
                  throw CE("Mailbox is shutdown");
                  ...
                  if (result.status != Cr.NS_OK)
                  {
                    ...
                    try {
                      for (let i = 0; i < localIds.length; i++)
                      {
                        ...
                        if (aMailbox.isOnline && (dirtyItem.flags & Ci.msqIEwsNativeItem.Dirty))
                        {
                          ...
                          else
                          {
                            ...
                            dirtyItem.raiseFlags(Ci.msqIEwsNativeItem.DeletedOnServerBit);
                            if (aListener)
                        ...
                        else
                        {
                          ...
                          dirtyItem.raiseFlags(Ci.msqIEwsNativeItem.Dirty);
                          let dsListener = new PromiseUtils.DatastoreListener();
                          aMailbox.datastore.putItem(dirtyItem, dsListener);
                          let result = yield dsListener.promise;
                          items.appendElement(dirtyItem, false); // get it this time around
                    ...
                    let results = yield Promise.all(batchPromises);
            ...
            let getItemResults = yield Promise.all(syncPromises);
  ...
  machineErrorResponse: function _machineErrorResponse(aListener, aResponseError, aResponseCode, aMessageText)
  {
    ...
    if (aResponseError == "PasswordMissing")
    {
      ...
      try {
        aContext.QueryInterface(Ci.msqIEwsNativeMailbox)
...
function _errorResponse(aRequest, aContext, aResponseError, aResponseCode, aMessageText)
{
  ...
  PromiseUtils.SoapResponse.prototype.errorResponse.call
    (this, aRequest, aContext, aResponseError, aResponseCode, aMessageText);
}

// utility functions
function promiseChangedItems(aFolder, aSyncState, aItemsPerSync)
{
  ...
  return soapResponse.promise.then(
    result => {result.items = items; return result});
}

var promiseCheckOnline = Task.async(function* taskCheckOnline(aMailbox, aListener)
{
  ...
  try {
    ...
    if (result.status != Cr.NS_OK && !aMailbox.didAutodiscover)
    {
      ...
      result = yield promiseAutodiscoverUrl(aMailbox, aListener);
    }
...
var promiseInitial = Task.async(function* taskInitial(aMailbox, aListener)
{
  let result = {status: Cr.NS_OK};
  try {
    do {
      ...
      if (aMailbox.needFolderDiscovery)
      {
        ...
        result = yield promiseDiscoverSubfolders(aMailbox, nativeRootFolder, aListener);
        if (result.status != Cr.NS_OK)
          break;
      }
...
var promiseGetDistinguishedIds = Task.async(function* taskGetDistinguishedIds(aMailbox, aListener)
{
  ...
  try {
    ...
    for (let id of gDistinguishedIds)
      folders.appendElement(aMailbox.getNativeFolder(id), false);
...
var promiseAutodiscoverUrl = Task.async(function* taskAutodiscoverUrl(aMailbox, aEventListener)
{
  let result = {status: Cr.NS_ERROR_FAILURE};
  try {
    result = yield PromiseUtils.promiseAutodiscover(aMailbox.email.length ? aMailbox.email : aMailbox.username,
    ...
    if (result.foundSite)
    {
      ...
      {
        aMailbox.ewsURL = url;
        result = yield promiseCheckOnline(aMailbox, aEventListener);
...
var promiseDiscoverSubfolders = Task.async(function* taskDiscoverSubfolders(aMailbox, aFolder, aListener)
{
  let result = {status: Cr.NS_ERROR_FAILURE};
  try
  {
...
var promiseProcessItem = Task.async( function* _processItem(aChangedItem, aGetAttachments, aListener, aItemsDeathGrip)
{ let result = {status: Cr.NS_OK};
  try {
    ...
    let isDeleted = aChangedItem.flags & (Ci.msqIEwsNativeItem.DeletedBit | Ci.msqIEwsNativeItem.DeletedOnServerBit);
         ...
         (itemClass.indexOf("IPM.Task") == 0) )
    {
      do
      {
        ...
        {
          let occurrence = occurrences.queryElementAt(j, Ci.msqIPropertyList).wrappedJSObject;
          if (!occurrence)
          {
    ...
    aChangedItem.clearFlags(Ci.msqIEwsNativeItem.AllOnServer);
    ...
    {
      if (!(aChangedItem.flags & Ci.msqIEwsNativeItem.Dirty) &&
           aChangedItem.processingFlags & Ci.msqIEwsNativeItem.DidNotChange)
      {
      ...
      else
      {
        ...
        if (aChangedItem.flags & Ci.msqIEwsNativeItem.Dirty)
          aChangedItem.clearFlags(Ci.msqIEwsNativeItem.Dirty);
        let dsListener = new PromiseUtils.DatastoreListener();

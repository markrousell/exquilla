...
function EwsNativeMailbox() {
  // cache of msqIEwsNativeFolder objects, referenced by folder ID.
...
EwsNativeMailbox.prototype = {
  QueryInterface: XPCOMUtils.generateQI([Ci.msqIEwsNativeMailbox,
                                         Ci.nsISupportsWeakReference]),
  classInfo:       XPCOMUtils.generateCI({
                     classID: Components.ID("{6F3A2233-4FF4-4810-8727-2DC658BA7600}"),
                     classDescription: "ExQuilla native mailbox",
                     contractID: "@mesquilla.com/ewsnativemailbox;1",
                     interfaces: [Ci.msqIEwsNativeMailbox],
                     flags: 0,
                   }),
  ...
  set datastoreDirectory(datastoreDirectory) {
    ...
    this._datastore = Cc["@mesquilla.com/ewsdatastore;1"]
  ...
  getItemAsync(aItemId, aListener) {
    ...
    GetItemListener.prototype.QueryInterface = function(iid) {
      if (iid.equals(Ci.nsISupports) ||
          iid.equals(Ci.msqIEwsEventListener))
        return this;
    ...
    GetItemListener.prototype.onEvent = function(aItem, aEvent, aData, aResult) {
      let item = (aItem ? aItem.QueryInterface(Ci.msqIEwsNativeItem) : null);
      ...
      switch (this._state)
      {
        ...
        case "GetProperties":
        {
          ...
          if (aEvent == "StatementComplete" &&
             ...
             (!(item.flags & (Ci.msqIEwsNativeItem.Dirty | Ci.msqIEwsNativeItem.NeedsResync))) &&
             (item.folderId))
          {
  ...
  removeItem(aNativeItem) { 
    ...
    if (!(aNativeItem.processingFlags & Ci.msqIEwsNativeItem.DeletedInDatastore))
    {

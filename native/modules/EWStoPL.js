...
// Implements conversions from DOM EWS XML documents to msqIPropertyList.
...
function plRecipientsIfXML(pl, name)
{
  ...
  if (toRecipients && toRecipients.length) {
    ...
    for (let i = 0; i < toRecipients.length; i++) {
      xml += plMailboxIfXML(toRecipients.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject);
    }
    xml += `</${name}>`;
  }
  return xml;
...
function itemToItemXML(item, isCreate) {
  ...
  if (!hasMimeContent) {
    ...
    if (bodyPL && bodyPL.length) {
      ...
      body = item.body;
      bodyIsHTML = item.flags & Ci.msqIEwsNativeItem.BodyIsHtml;
    }
    if (body)
...
function itemToContactXML(item, isCreate)
{
  ...
  if (EmailAddressesEntries && EmailAddressesEntries.length) {
    xml += `<EmailAddresses>`;
    for (let index = 0; index < EmailAddressesEntries.length; index++) {
      let entry = EmailAddressesEntries.queryElementAt(index, Ci.msqIPropertyList).wrappedJSObject;
  ...
  if (PhysicalAddressesEntries && PhysicalAddressesEntries.length) {
    ...
    for (let index = 0; index < PhysicalAddressesEntries.length; index++) {
      let entry = PhysicalAddressesEntries.queryElementAt(index, Ci.msqIPropertyList).wrappedJSObject;
      let key = entry.getAString("$attributes/Key");
      xml += `<Entry Key="${key}">`;
  ...
  if (PhoneNumbersEntries && PhoneNumbersEntries.length) {
    xml += `<PhoneNumbers>`;
    for (let index = 0; index < PhoneNumbersEntries.length; index++) {
      let entry = PhoneNumbersEntries.queryElementAt(index, Ci.msqIPropertyList).wrappedJSObject;
  ...
  if (ImAddressesEntries && ImAddressesEntries.length) {
    xml += `<ImAddresses>`;
    for (let index = 0; index < ImAddressesEntries.length; index++) {
      let entry = ImAddressesEntries.queryElementAt(index, Ci.msqIPropertyList).wrappedJSObject;

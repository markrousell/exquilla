...
const nsIDataType = Ci.nsIDataType;
    ...
    switch (state)
    {
      ...
      case FIND_VALUE:
        ...
        if (cur == '{')
        {
          ...
          result.appendElement(name, valueAsPL.QueryInterface(Ci.msqIPropertyList));
...
// msqIPropertyEnumerator implementation
...
PropertyEnumerator.prototype = {
  ...
  QueryInterface:   XPCOMUtils.generateQI([Ci.msqIPropertyEnumerator]),
...
PropertyList.prototype = {
  ...
  classInfo:        XPCOMUtils.generateCI({classID: null,
                                           contractID: null,
                                           classDescription: "ExQuilla Property List",
                                           interfaces: [Ci.msqIPropertyList]}),
  QueryInterface:   XPCOMUtils.generateQI([Ci.msqIPropertyList]),
  ...
  getPropertyList: function(aName) {
    ...
    try {
      ...
      if (value)
        return value.QueryInterface(Ci.msqIPropertyList);
  ...
  getAString: function(aName) {
    try {
      ...
      if ( !(safeInstanceOf(result, Ci.msqIPropertyList)) // which has no value
           && (result !== null) ) // because "null" means no value
        return result + "";
    } catch (e) {}
    return "";
  },
  ...
  getPropertyLists: function(aName) {
    ...
    if (slashIndex >= 0) {
      ...
      if (!headPL) {
        ...
        return Cc["@mozilla.org/array;1"].createInstance(Ci.nsIMutableArray);
      }
    ...
    for (let index = 0; index < this._names.length; index++) {
      if (this._names[index] === aName)
        values.appendElement(this._values[index], false);
    }
    return values;
  },

  /* msqIPropertyList getPropertyListByAttribute (in AString aName, in AString aAttribute, in AString aValue); */
  getPropertyListByAttribute: function(aName, aAttribute, aValue) {
    ...
    for (let i = 0; i < propertyLists.length; i++) {
      let pl = null;
      try {
        pl = propertyLists.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject;
      } catch (e) {}
      if (pl) {
        let attributes = pl.getPropertyList("$attributes");
        if (safeInstanceOf(attributes, Ci.msqIPropertyList)) {
 ...
 getNameAt: function(aIndex) {
  ...
  getPropertyListAt: function(aIndex) {
    return this.getValueAt(aIndex).QueryInterface(Ci.msqIPropertyList);
  },
  ...
  /* msqIPropertyList clone (in msqIStringArray aExclude); */
  clone: function(aExclude) {
    let clonedList = newPL();
    for (let i = 0; i < this._names.length; i++) {
      if (aExclude && (aExclude.indexOf(this._names[i]) != Ci.msqIStringArray.noIndex))
        continue;

      let value = this._values[i];
      if (safeInstanceOf(value, Ci.msqIPropertyList))
  ...
  get PLON() {
    ...
    for (let i = 0; i < this._names.length; i++) {
      ...
      if (typeof value === 'boolean') {
        type = Ci.nsIDataType.VTYPE_BOOL;
        outvalue = value ? '"1"' : '"0"';
      }
      else if (Number.isInteger(value)) {
        type = Ci.nsIDataType.VTYPE_INT32;
        outvalue = '"' + Number.parseInt(value) + '"';
      }
      else if (safeInstanceOf(value, Ci.msqIPropertyList)) {
        type = Ci.nsIDataType.VTYPE_INTERFACE_IS;
        outvalue = value.PLON;
      }
      else {
        // defaults to string
        type = Ci.nsIDataType.VTYPE_ASTRING;

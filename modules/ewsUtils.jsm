...
XPCOMUtils.defineLazyModuleGetter(this, "jsmime", "resource:///modules/jsmime.jsm");
XPCOMUtils.defineLazyModuleGetter(this, "MailServices", "resource:///modules/mailServices.js");
...
function stringPL(aList, aPrefix)
{
  ...
  while (enumerator.hasMoreElements())
  {
    ...
    {
      result += 'property <' + prefix + name + '> is msqIPropertyList' + '\n';
...
function dumpXMLResponse(aElement) {
  ...
  if (!(element instanceof Ci.nsIDOMElement)) {
    dump(se("element is not an nsIDOMElement\n"));
    return;
  }
  ...
  nextStep: function asc_nextStep(aStatus)
  {
    ...
    function _nextStep(aaStatus, curGenerator)
    {
      try {
        ...
        if (ex != StopIteration) {
          re('ExQuilla generator exception: ' + ex);
          curGenerator.next();
        }
      }
    }
  ...
  function addAddressesHeader(headerName, propertyName) {
    ...
    if (pls && pls.length) {
      ...
      for (let i = 0; i < pls.length; i++) {
        let contact = pls.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject;
...
function safeGetInterface(obj, iface) {
  let jsXPObject = null;
  try {
...
Utils.safeGetInterface = safeGetInterface;
...
function RotatingFileAppender(path, formatter, maxSize=4*ONE_MEGABYTE) {
  Log.FileAppender.call(this, path, formatter);
...
RotatingFileAppender.prototype = {
    ...
    return this._openFile()
      ...
      .then(fileStat => {
        ...
        if (pendingMessages) {
          ...
          this._lastWritePromise = this._file.write(byteArray);
        }
      });
  },
  ...
  doAppend: function (formatted) {
    ...
    this._rotateFilePromise = this.reset().then(_ => {
      this._rotateFilePromise = null;
      if (!this._file)
        this._fileReadyPromise = this._getFile();
    });
...
var formatter = null;
function EwsFormatter() {
  Log.BasicFormatter.call(this);
}
EwsFormatter.prototype = {
  __proto__: Log.BasicFormatter.prototype,
...
let promiseFlushLogging = Task.async(function* ()
{
  if (!fileAppender) {
  ...
  if (fileAppender._file) {
    dl("starting close");
    yield fileAppender._file.close();
    dl("finished close");

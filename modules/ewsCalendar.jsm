...
var EXPORTED_SYMBOLS = ["EwsCalendar", "EwsCalUtils"];
  ...
  case 'Begin':
    {
      if (this.mNewItem.parentItem == this.mNewItem)
      {
        ...
        //this.mNativeItem.raiseFlags(Ci.msqIEwsNativeItem.UpdatedLocally);
      ...
      else
      {
        ...
        ///this.mNativeItem.raiseFlags(Ci.msqIEwsNativeItem.UpdatedLocally); // NewLocally?
      ...
      if (aEvent == 'StopMachine')
      {
        if (result == Cr.NS_OK || result == Cr.NS_ERROR_ALREADY_INITIALIZED)
        {
          ...
          else
          {
            ...
            parentNativeItem.raiseFlags(Ci.msqIEwsNativeItem.UpdatedLocally);
    ...
    if (this.mState == 'Begin')
    {
      ...
      if (this.mItem.parentItem != this.mItem)
      {
        ...
        if (nativeItem.properties) // that is, we got a real item
        {
          ...
          if (realItemId.length && !(nativeItem.flags & Ci.msqIEwsNativeItem.HasTempId))
          {
      ...
      //ecalLog().debug('hasTempId is ' + this.mItem.flags & Ci.msqIEwsNativeItem.HasTempId);
  ...
  if (aEvent == 'StopMachine')
  {
    ...
    while (true)
    {
      ...
      if (this.mState == 'WaitCreateRecurrence')
      {
        ...
        ewsItem.raiseFlags(Ci.msqIEwsNativeItem.UpdatedLocally); // NewLocally?
        this.mState = 'WaitCreateRecurrence';
  ...
  while (true)
  {
    ...
    if (this.mState == 'WaitUpdate')
    {
      if (aEvent == 'ItemChanged')
      {
        ...
        {
          if ( oldFlags & (Ci.msqIEwsNativeItem.NewOnServerBit |
          ...
          else if ((oldFlags & Ci.msqIEwsNativeItem.DeletedOnServerBit) && oldItem)
          {
        ...
        aData.clearFlags(Ci.msqIEwsNativeItem.AllOnServer);
        break;
      }
...
{ try {
  ...
  while (true)
  {
    ...
    if (this.mState == 'WaitIds')
    {
      if (aEvent == 'StatementComplete')
      {
        this.mIds = aData.QueryInterface(Ci.msqIStringArray).wrappedJSObject;
        ...
        else
        {
          ...
          if (!ewsItem.properties.getAString("RecurrenceId").length)
          {
                ...
                calItem.getProperty('SEQUENCE') != ewsItem.changeKey)
            {
              ...
              else if (!(ewsItem.flags & Ci.msqIEwsNativeItem.DeletedLocally))
              {
...
// msqIEwsCalUtils implementation
function EwsCalUtils()
{}

EwsCalUtils.prototype =
{
  QueryInterface: XPCOMUtils.generateQI([Ci.msqIEwsCalUtils]),

  itemToCalendar: function _itemToCalendar(aNativeItem)
  {
...
{
  QueryInterface:   XPCOMUtils.generateQI([Ci.calICalendar, Ci.calISchedulingSupport]),
...
{ try {
  ...
  if (recurrence)
  {
    ...
    if (deletedOccurrences)
    {
      ...
      {
        let startDateString = deletedOccurrencesArray.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject
                                                     .getAString("Start");
    ...
    if (modifiedOccurrences)
    {
      ...
      for (let i = 0; occurrencesArray && i < occurrencesArray.length; i++)
      { try {
        let occurrence = occurrencesArray.queryElementAt(i, Ci.msqIPropertyList).wrappedJSObject;
        ...
        if (!modifiedEwsItem.properties)
        {
          ...
          if (!(aNativeItem.flags & Ci.msqIEwsNativeItem.Dirty))
          {
            ecalLog().info('recurrence has no properties, marking dirty');
            aNativeItem.raiseFlags(Ci.msqIEwsNativeItem.Dirty);
            aNativeItem.mailbox.datastore.putItem(aNativeItem, null);
          }
        }
        else
